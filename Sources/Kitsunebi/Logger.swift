...
public protocol LoggerDelegate: AnyObject {
  func didEmit(log: String, source: String, userInfo: [String: Any?])
}
...
public class Logger {
  ...
  func log(_ log: String, source: String, userInfo: [String: Any?] = [:]) {
    logQueue.async { [weak self] in
      self?.delegate?.didEmit(log: log, source: source, userInfo: userInfo)
    }
  }
}
